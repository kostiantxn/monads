...
[AsyncMethodBuilder(typeof(IdentityMonadMethodBuilder<>))]
public partial class Identity<T>
{
    public IdentityMonadAwaiter<T> GetAwaiter() =>
        new(this);
}

public abstract class IdentityMonadAwaiter : INotifyCompletion
{
    private bool _set;
    ...
    public abstract Identity<U> Bind<U>(Func<object, Identity<U>> map);
...
public class IdentityMonadAwaiter<T>(Identity<T> container) : IdentityMonadAwaiter
{
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public T GetResult() =>
        (T) Result;

    public override Identity<U> Bind<U>(Func<object, Identity<U>> map) =>
        container.Bind((T x) => map(x!));
}

public class IdentityMonadMethodBuilder<T>
{
    public static IdentityMonadMethodBuilder<T> Create() =>
        new();

    public void SetResult(T result) =>
        Task = Identity<T>.Return(result);

    public Identity<T> Task { get; protected set; } = default!;
    ...
    public void AwaitOnCompleted<TAwaiter, TStateMachine>(ref TAwaiter awaiter, ref TStateMachine stateMachine)
        where TAwaiter : IdentityMonadAwaiter, INotifyCompletion
        where TStateMachine : IAsyncStateMachine
    {
    ...
    public void AwaitUnsafeOnCompleted<TAwaiter, TStateMachine>(ref TAwaiter awaiter, ref TStateMachine stateMachine)
        where TAwaiter : IdentityMonadAwaiter, ICriticalNotifyCompletion
        where TStateMachine : IAsyncStateMachine =>
